...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > lastScrollY) {
        setShowNavbar(false); // নিচে নামলে hide
      } else {
        setShowNavbar(true); // উপরে উঠলে show
      }
  ...
  return (
    <div
      className={`fixed left-0 right-0 z-50 flex justify-center transition-transform duration-500 ${
        showNavbar ? "translate-y-0" : "-translate-y-full"
      }`}
    >
      <div className="navbar bg-base-100 shadow-xl rounded-2xl px-4 sm:px-8 transition-all duration-300 hover:scale-[1.01]">
